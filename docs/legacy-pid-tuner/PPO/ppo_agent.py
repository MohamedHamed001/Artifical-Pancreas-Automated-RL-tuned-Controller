...
class PPOTunner(object):
    ...
    def train(self, max_episode_num, plot=False, on_wandb = False):
        ...
        self.actor.save_weights(os.path.join(save_dir, 'pid_actor.h5'))
        self.critic.save_weights(os.path.join(save_dir, 'pid_critic.h5'))

        np.savetxt(os.path.join(save_dir, 'pid_epi_reward.txt'), self.save_epi_reward)
